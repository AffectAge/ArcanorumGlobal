...
- MVP-механика населения провинций: `populationTotal` и процентные распределения по культурам/идеологиям/религиям/расам/профессиям.
- Кнопка `Население` в левой панели и модальное окно статистики населения (страна/мир) в стиле `ContentPanel`.
- В модалке населения добавлено верхнее подменю разделов как в `ContentPanel`: `Основная информация` и `Логотип и стиль`.
- В верхние вкладки модалки населения добавлены отдельные статистические разделы: `Религии`, `Культуры`, `Профессии`, `Идеологии`, `Расы`.
- В модалке населения для категорий добавлен layout:
  - слева `Apache ECharts` график типа `Pie with Scrollable Legend`,
  - справа selectable-список элементов выбранной категории.
- В админ-панель добавлен раздел `Управление населением`:
  - генерация населения по scope (`провинция` / `страна` / `мир`),
  - выбор стратегии генерации (`случайные` или `заданные` пропорции),
  - очистка населения по scope,
  - ручное редактирование населения выбранной провинции.
- Endpoint сноса построек `POST /country/build/demolish` со списанием очков строительства.
...
- Сжат payload WS-дельт (`mask` + короткие ключи `c/o/n/p/z/u`) для уменьшения размера сообщений.
...
- Full snapshot `worldBase` в дельта-пайплайне заменен на partial snapshot по dirty-sections (`mask`): сервер теперь копирует только изменяемые ветки перед diff (`c/o/n/p/z/u`), что снижает CPU/GC нагрузку в hot-path (`resolveTurn`, колонизация, rename, админ-операции).
- В `resolveTurn` добавлен серверный апдейт населения на каждом ходу (рождаемость/смертность); процентные распределения населения не меняются автоматически.
- В экономике игры добавлена настройка `demolitionCostConstructionPercent` (по умолчанию `20%`) для стоимости сноса постройки в очках строительства.
- В окне `Постройки` кнопка сноса для построенных зданий теперь активна и использует подтверждение с отображением стоимости сноса.
- Система построек переведена с уровней на реальные инстансы зданий (`instanceId`): завершение строительства добавляет отдельный объект здания, а снос удаляет конкретный инстанс.
- Окно `Постройки` переработано под формат `Индустрия`: добавлены компактные фильтры, сортировка, счетчик результатов, 3-колоночная сетка карточек и встроенный разворачиваемый блок `Экономика` в карточке здания.
