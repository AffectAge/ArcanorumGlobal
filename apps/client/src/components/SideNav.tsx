...
export function SideNav() {
  return (
    <aside className="pointer-events-auto absolute left-4 top-32 z-40 hidden flex-col gap-2 xl:flex">
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            ...
            />
            <span className="relative z-10 ml-7 max-w-0 overflow-hidden whitespace-nowrap text-xs font-medium text-arc-accent opacity-0 transition-all duration-150 group-hover:max-w-[110px] group-hover:opacity-100">
              {item.label}
            </span>
          </motion.button>
        );
